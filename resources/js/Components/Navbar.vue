<template>
  <nav
    ...
    :class="isHeroMode ? 'bg-transparent hero-nav' : 'bg-white shadow-sm border-b border-green-100/60'"
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16 md:h-20">
        ...
        <div class="hidden md:flex items-center gap-8">
          ...
          <Link :href="route('offers')"      class="nav-link" :class="isActive('offers')">Offers</Link>
        ...
        <div class="flex items-center gap-1">
          ...
          <button @click="toggleSearch"
            class="icon-btn" :class="searchOpen ? 'text-primary bg-green-100/70' : ''">
          ...
          <button @click="openCart" class="relative icon-btn">
            ...
            <span v-if="cart_count > 0"
              class="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 rounded-full gradient-green text-white text-[10px] flex items-center justify-center font-bold leading-none shadow-sm">
              {{ cart_count }}
            </span>
          </button>
          ...
          <div class="relative" v-if="auth?.user" ref="userMenuRef">
            <button @click="userMenuOpen = !userMenuOpen"
              class="flex items-center gap-1.5 px-1.5 py-1 rounded-xl hover:bg-green-50 transition-all duration-200 ml-1">
              <div class="w-8 h-8 rounded-full gradient-green flex items-center justify-center text-white text-sm font-semibold shadow-sm">
            ...
            <transition name="dropdown">
              <div v-if="userMenuOpen"
                class="absolute right-0 top-12 bg-white rounded-2xl shadow-xl border border-green-100/60 py-2 z-50 min-w-52.5">
                <div class="px-4 py-2.5 border-b border-green-50 mb-1">
                ...
                <div class="border-t border-green-50 my-1"></div>
            ...
            <div class="relative w-full sm:w-80 md:w-96" ref="searchContainer">
              ...
              <transition name="dropdown">
                <div v-if="showResults"
                  class="absolute top-full mt-2 left-0 right-0 bg-white rounded-2xl shadow-xl border border-green-100/60 overflow-hidden z-50">
                  ...
                  <template v-if="searchResults.length > 0">
                    <Link
                      ...
                      @click="closeSearch"
                      class="flex items-center gap-3 px-4 py-3 hover:bg-green-50/60 transition-colors group">
                      <img :src="product.featured_image_url" :alt="product.name"
                        class="w-10 h-10 object-cover rounded-xl bg-green-50 shrink-0">
                    ...
                    <div class="border-t border-green-50">
                      <button @click="submitSearch"
                        class="flex items-center justify-center gap-1.5 w-full px-4 py-2.5 text-xs text-primary font-medium hover:bg-green-50/50 transition-colors">
      ...
      <transition name="slide-down">
        <div v-if="mobileMenuOpen"
          class="md:hidden bg-white rounded-2xl shadow-xl border border-green-100/60 p-3 mb-4">
          <div class="flex flex-col gap-0.5">
            ...
            <Link :href="route('offers')"     class="mobile-nav-link" @click="mobileMenuOpen = false">
              <TagIcon class="w-5 h-5" /> Offers & Deals
            </Link>
            <Link :href="route('about')"      class="mobile-nav-link" @click="mobileMenuOpen = false">
              <InformationCircleIcon class="w-5 h-5" /> About Us
            </Link>
            <Link :href="route('contact')"    class="mobile-nav-link" @click="mobileMenuOpen = false">
              <PhoneIcon class="w-5 h-5" /> Contact
            </Link>
            <div class="border-t border-green-100 my-1.5"></div>
            <Link v-if="!auth?.user" :href="route('login')"    class="btn-primary text-center" @click="mobileMenuOpen = false">Login</Link>
            <Link v-if="!auth?.user" :href="route('register')" class="btn-secondary text-center mt-1" @click="mobileMenuOpen = false">Register</Link>
          </div>
        </div>
      </transition>
    </div>
...
const handleOutsideClick = (e) => {
  ...
  if (searchContainer.value && !searchContainer.value.contains(e.target)) showResults.value = false
}
...
.icon-btn {
  @apply p-2.5 rounded-xl hover:bg-green-50 text-muted hover:text-primary transition-all duration-200;
}
...
.mobile-nav-link {
  @apply flex items-center gap-3 px-4 py-2.5 rounded-xl text-charcoal hover:bg-green-50 hover:text-primary transition-all duration-200 font-medium text-sm;
}
.dropdown-item {
  @apply flex items-center gap-3 px-4 py-2 text-sm text-charcoal hover:bg-green-50 hover:text-primary transition-colors;
}
...
.search-input::placeholder { color: #9d93a0; }
.search-input:focus {
  border-color: #2E9D63;
  background: #fff;
  box-shadow: 0 0 0 3px rgba(46, 157, 99, 0.12);
}
